// Seed Strapi DB with accelerator content (pricing, downloads, articles, global)
...
async function api(path, opts = {}) {
  ...
  if (!res.ok) {
    ...
    throw new Error('Strapi request failed: ' + path);
  }
...
function blocksToMarkdown(blocks) {
  return blocks
    .map((b) => {
      const text = (b.children || []).map((c) => c?.children?.[0]?.text || c?.text || '').join('');
      if (b.type === 'heading') return `${'#'.repeat(b.level || 2)} ${text}\n\n`;
      if (b.type === 'paragraph') return `${text}\n\n`;
      if (b.type === 'list') {
        const items = (b.children || []).map((li) => (li.children || []).map((c) => c?.children?.[0]?.text || c?.text || '').join(''));
        const prefix = b.format === 'ordered' ? '1.' : '-';
        return items.map((it) => `${prefix} ${it}`).join('\n') + '\n\n';
      }
...
const globalData = {
  ...
  defaultSeoDescription: '覆盖 90+ 国家节点，4G/5G 专线，IPLC 内网专线。一键连接，稳定不掉线。',
  footerCopy: '© 星云加速 · 隐私保护工具，仅供合法跨境办公使用。',
};

console.log('\n1/4 Seeding global-setting...');
const g = await api('/api/global-setting', { method: 'PUT', headers: auth, body: { data: globalData } });
console.log('   Saved.');
...
console.log('\n2/4 Seeding pricing-plans...');
for (const p of plans) {
  const r = await api('/api/pricing-plans', { method: 'POST', headers: auth, body: { data: p } });
  console.log('   -', p.name, '-> id =', r?.data?.id);
}
...
console.log('\n3/4 Seeding client-downloads...');
for (const d of downloads) {
  const r = await api('/api/client-downloads', { method: 'POST', headers: auth, body: { data: d } });
  console.log('   -', d.platform, '-> id =', r?.data?.id);
}
...
const articles = [
  {
    ...
    content: [
      ...
      { type: 'paragraph', children: [{ text: '严格来说，"加速"指的是通过专线、协议优化和路由选择，让跨境的网络请求更快、更稳；"翻墙"只是其中一个典型场景——除了翻墙，跨境办公、远程协作、海外购物、视频会议、流媒体观看，都属于加速的范畴。' }] },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'paragraph', children: [{ text: '三者都是"不走公共互联网"的专线方案，只是层级和成本不同。' }] },
      { type: 'heading', level: 3, children: [{ text: 'IPLC（International Private Leased Circuit）' }] },
      { type: 'paragraph', children: [{ text: '国际私用专线，端到端独占，丢包率极低。适合对延迟极其敏感的游戏玩家、交易员。' }] },
      { type: 'heading', level: 3, children: [{ text: 'IEPL（International Ethernet Private Line）' }] },
      { type: 'paragraph', children: [{ text: '以太网级专线，比 IPLC 便宜一些，企业用得更多。日常办公、视频会议完全够用。' }] },
      { type: 'heading', level: 3, children: [{ text: '5G 专线' }] },
      { type: 'paragraph', children: [{ text: '基于 5G 移动网络的优化通道，速度快、部署灵活，适合个人手机端用户。' }] },
    ],
  },
  {
    ...
    content: [
      ...
      { type: 'paragraph', children: [{ text: 'Netflix 同时检测 IP 归属、DNS 出口、时区与浏览器语言。任何一个对不上，都会"露馅"。' }] },
...
console.log('\n4/4 Seeding articles...');
for (const a of articles) {
  const data = { ...a, content: blocksToMarkdown(a.content) };
  const r = await api('/api/articles', { method: 'POST', headers: auth, body: { data } });
  console.log('   -', a.title, '-> id =', r?.data?.id);
}

console.log('\n✅ Done. Restart Astro to see the new content at http://127.0.0.1:4321/');
